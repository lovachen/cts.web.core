    ...
    public static class Chinese2PinYin
    {
        ...
        private static string GetPinyin(string input)
        {
            ...
            else if (chrAsc <= lastOfOneLevelChCode)
            {
                ...
                for (int aPos = 11; aPos >= 0; aPos--)
                {
                    ...
                    // 从最后的块开始扫描,如果机内碼大于块的第一个机内碼,说明在此块中
                    if (chrAsc >= pyValue[aboutPos])
                    {
                        // 遍历块中的每个音节机内碼,从最后的音节机内碼开始扫描,
                        // 如果音节内碼小于机内碼,则取此音节
            ...
            {
                int pos = Array.IndexOf(otherChinese, input);
        ...
        public static string Convert(string hzString)
        {
            ...
            for (int j = 0; j < noWChar.Length; j++)
            {
                ...
                {
                    pySB.Append(GetPinyin(noWChar[j].ToString()));
                }
                // 非中文字符
                else
                {
                    //数字和英文直接返回小写
                    if (en_regex.IsMatch(noWChar[j].ToString()))
                        pySB.Append(noWChar[j].ToString().ToLower());
                }
            }
            return pySB.ToString();
        }
        ...
        public static string ConvertWithBlank(string hzString)
        {
            ...
            for (int j = 0; j < noWChar.Length; j++)
            {
                ...
                if (regex.IsMatch(noWChar[j].ToString()))
                {
                    pySB.Append(" ").Append(GetPinyin(noWChar[j].ToString()));
                }
                // 非中文字符
                else
                {
        ...
        public static string ConvertWithSplitChar(string hzString, char splitChar)
        {
            ...
            for (int j = 0; j < noWChar.Length; j++)
            {
                ...
                {
                    Encoding encoding = Encoding.GetEncoding("GB2312");
                    array = encoding.GetBytes(noWChar[j].ToString());
                    i1 = (short)(array[0]);
                    i2 = (short)(array[1]);
